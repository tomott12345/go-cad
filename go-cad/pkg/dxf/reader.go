...
// floatOK returns (value, true) when the group-code value is a valid float64,
// or (0, false) when it is not parseable. Use instead of float() when the
// caller wants to emit a warning for a bad value.
func (g gc) floatOK() (float64, bool) {
        v, err := strconv.ParseFloat(strings.TrimSpace(g.val), 64)
        return v, err == nil
}
...
type parser struct {
        ...
        acadVer  string                    // e.g. "AC1009" (R12) or "AC1015" (R2000)
...
func (p *parser) parse() {
        for {
                ...
                case "TABLES":
                        p.parseTables()
                        // Apply layer properties immediately so entity parsing can look
                        // them up by name with correct color/linetype/flags. DXF guarantees
                        // TABLES comes before ENTITIES; applyLayers is idempotent.
                        p.applyLayers()
                case "BLOCKS":
                        p.parseBlocks()
                case "ENTITIES":
        ...
        // Second pass: update any layers that were auto-created by entity references
        // before the TABLES section was encountered (malformed/reordered DXF files).
        p.applyLayers()
}

func (p *parser) parseHeader() {
        // Parse $ACADVER so we can record the DXF version. Other header variables
        // are consumed and discarded; we do not need them for entity parsing.
        for {
                g, ok := p.next()
                if !ok || (g.code == 0 && strings.EqualFold(g.str(), "ENDSEC")) {
                        return
                }
                // $ACADVER is group-code 9, value is the variable name;
                // the next pair (code 1) is the version string.
                if g.code == 9 && g.str() == "$ACADVER" {
                        vg, ok2 := p.next()
                        if ok2 && vg.code == 1 {
                                p.acadVer = vg.str() // e.g. "AC1009" or "AC1015"
                        }
                }
        }
}
...
// ─── Warning helper for coordinate fields ─────────────────────────────────────

// coordFloat parses a float from gc g, emitting a warning on the parser if the
// value is not a valid number. entity is used in the warning message.
func (p *parser) coordFloat(g gc, entity string) float64 {
        v, ok := g.floatOK()
        if !ok {
                p.warn("%s: group code %d is not a valid number (%q) — using 0", entity, g.code, g.val)
        }
        return v
}
...
func (p *parser) parseLine() (document.Entity, bool) {
        ...
        for _, g := range codes {
                ...
                case 8:
                        layerName = g.str()
                case 10:
                        e.X1 = p.coordFloat(g, "LINE")
                case 20:
                        e.Y1 = -p.coordFloat(g, "LINE")
                case 11:
                        e.X2 = p.coordFloat(g, "LINE")
                case 21:
                        e.Y2 = -p.coordFloat(g, "LINE")
                case 62:
                        aci := g.int()
                        if aci != 0 {
...
func (p *parser) parseCircle() (document.Entity, bool) {
        ...
        for _, g := range codes {
                ...
                case 8:
                        layerName = g.str()
                case 10:
                        e.CX = p.coordFloat(g, "CIRCLE")
                case 20:
                        e.CY = -p.coordFloat(g, "CIRCLE")
                case 40:
                        e.R = p.coordFloat(g, "CIRCLE")
                case 62:
                        aci := g.int()
                        if aci != 0 {
                                e.Color = aciToRGB(absInt(aci))
...
func (p *parser) parseArc() (document.Entity, bool) {
        ...
        for _, g := range codes {
                ...
                case 8:
                        layerName = g.str()
                case 10:
                        e.CX = p.coordFloat(g, "ARC")
                case 20:
                        e.CY = -p.coordFloat(g, "ARC")
                case 40:
                        e.R = p.coordFloat(g, "ARC")
                case 50:
                        startDXF = p.coordFloat(g, "ARC")
                case 51:
                        endDXF = p.coordFloat(g, "ARC")
                case 62:
                        aci := g.int()
                        if aci != 0 {
                                e.Color = aciToRGB(absInt(aci))
...
// applyLayers is idempotent. It creates document layers for names in p.layers
// that do not yet exist, and updates the properties of layers that were already
// auto-created (by entity references before the TABLES section was seen). This
// ensures that color, linetype, visible, and locked flags from the LAYER table
// always win over the defaults used when a layer is first created on demand.
func (p *parser) applyLayers() {
        for name, info := range p.layers {
                if name == "0" {
                        continue
                }
                if existingID, alreadyCreated := p.layerIDs[name]; alreadyCreated {
                        // Layer was auto-created before TABLES was processed — update all
                        // properties to match what the DXF LAYER table says.
                        p.doc.SetLayerColor(existingID, info.color)
                        p.doc.SetLayerLineType(existingID, info.lineTyp)
                        p.doc.SetLayerVisible(existingID, info.visible)
                        p.doc.SetLayerLocked(existingID, info.locked)
                } else {
                        // Create the layer with the correct properties from the LAYER table.
                        id := p.doc.AddLayer(name, info.color, info.lineTyp, 0.25)
                        p.layerIDs[name] = id
                        p.doc.SetLayerVisible(id, info.visible)
                        p.doc.SetLayerLocked(id, info.locked)
                }
        }
}
