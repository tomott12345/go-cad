...
func TestCoincident(t *testing.T) {
	pts := ptrs(geometry.Point{0, 0}, geometry.Point{4, 4})
...
func TestHorizontal(t *testing.T) {
	pts := ptrs(geometry.Point{0, 1}, geometry.Point{5, 3})
...
func TestVertical(t *testing.T) {
	pts := ptrs(geometry.Point{1, 0}, geometry.Point{3, 5})
...
func TestFixed(t *testing.T) {
	target := geometry.Point{7, 8}
	pts := ptrs(geometry.Point{0, 0})
...
func TestParallel(t *testing.T) {
	...
	pts := ptrs(
		...
		geometry.Point{0, 2}, geometry.Point{5, 3},
	)
...
func TestPerpendicular(t *testing.T) {
	...
	pts := ptrs(
		geometry.Point{0, 0}, geometry.Point{5, 0},
		geometry.Point{2, 0}, geometry.Point{3, 1}, // slightly off perpendicular
	)
...
func TestEqualLength(t *testing.T) {
	pts := ptrs(
		geometry.Point{0, 0}, geometry.Point{3, 0}, // len 3
		geometry.Point{0, 5}, geometry.Point{7, 5}, // len 7
	)
...
func TestMidpoint(t *testing.T) {
	pts := ptrs(
		geometry.Point{0, 0},
		geometry.Point{10, 0},
		geometry.Point{3, 0}, // incorrect midpoint
	)
...
func TestSymmetric(t *testing.T) {
	...
	pts := ptrs(
		geometry.Point{0, 0}, geometry.Point{10, 0}, // axis along x-axis
		geometry.Point{3, 4},  // A
		geometry.Point{3, 1},  // B (should become mirror of A)
	)
...
func TestCombinedConstraints(t *testing.T) {
	...
	pts := ptrs(
		geometry.Point{0, 0},   // 0: origin (fixed)
		geometry.Point{5.1, 0.2}, // 1: should become (5,0)
		geometry.Point{5.2, 4.8}, // 2: should become (5,5)
		geometry.Point{0.1, 5.3}, // 3: should become (0,5)
	)
	constraints := []Constraint{
		FixedConstraint{Index: 0, Position: geometry.Point{0, 0}},
	...
	// Origin fixed
	if !pts[0].Near(geometry.Point{0, 0}) {
...
func TestSolveResult_NotConverged(t *testing.T) {
	...
	pt0 := geometry.Point{0, 0}
	pt1 := geometry.Point{1, 1}
	pts := []*geometry.Point{&pt0, &pt1}
	constraints := []Constraint{
		FixedConstraint{Index: 0, Position: geometry.Point{0, 0}},
		FixedConstraint{Index: 1, Position: geometry.Point{1, 1}},
